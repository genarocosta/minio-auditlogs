...
es = Elasticsearch(
    [ELASTICSEARCH_URL], request_timeout=30, basic_auth=(es_user, es_password)
)
...
def search_indexes(params):
    ...
    bodySearch = {"from": page * size, "size": size, "sort": []}
    ...
    response = es.search(index=index_pattern, body=bodySearch)
    result = []
...
def get_datalist(data):
    ...
    for rec in data:
        try:
            ...
            doc = {"_index": index, "_op_type": "index", "_source": erec}
            edb.append(doc)
        except Exception as e:
...
def query():
    ...
    return Response(
        json.dumps(data), status=200, headers={"Content-Type": "application/json"}
    )
